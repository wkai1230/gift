...
public class ListShowActivity extends BaseActivity {
    @Bind(R.id.recycle_view_show_list)
    RecyclerView rvShow;
    @Bind(R.id.tv_show_list)
    TextView tvShow;
    @Bind(R.id.btn_show_list)
    ImageButton btnBack;
    @Bind(R.id.ib_show_shear)
    ImageView ibShear;
    ...
    private void initListener() {
        ...
        ibShear.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ShearShow.show(ListShowActivity.this);
            }
        });
    }
        ...
        if (dateEntity == null) {
            OkHttpTool.newInstance(this).okGet(Constants.HOME_LIST_SHOW_HEADER + id + Constants.HOME_LIST_SHOW_END
                ...
                public void onSuccess(ShowListInfo resultInfo) {
                    ...
                    tvShow.setText(dateEntity.getTitle());
